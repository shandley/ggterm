...
ggterm addresses these gaps through a complete Grammar of Graphics implementation featuring 68 geometry types, structured plot history with provenance tracking, and first-class support for AI agent integration.
...
ggterm implements 68 geometry types organized by analytical field:
...
- **Hierarchical:** `geom_treemap`, `geom_sankey` (flow), `geom_flame`/`geom_icicle` (profiling)
...
The 68 geometry types cover analytical needs across fields. Genomics researchers can create volcano plots and Manhattan plots directly on the analysis server. Clinical researchers can generate Kaplan-Meier curves and forest plots during data review. Statisticians can assess model assumptions with Q-Q plots and residual diagnostics. This field coverage reduces context-switching between exploration and specialized visualization tools.
...
2. **Additional scientific fields:** Phylogenetic trees, genome browsers, and mass spectrometry visualizations would extend coverage to additional research areas.
...
**Figure 2.** Geometry type coverage across analytical fields: (A) taxonomy of 68 geometry types organized by purpose, (B) examples of scientific visualizations including volcano plot, Kaplan-Meier curve, and forest plot.
